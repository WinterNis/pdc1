...
class DocumentRegistry:
    ...
    def add_doc(self, doc_id, text, tokenCount):
        if doc_id not in self.document_registry:
            with open(self.doc_file_path, 'ab') as doc_file:
                start = doc_file.tell()
                doc_file.write(zlib.compress(str.encode(text)))
    ...
    def access_doc(self, doc_id):
        with open(self.doc_file_path, 'rb') as doc_file:
            doc = self.document_registry[doc_id]
            doc_file.seek(doc[0])
            text = zlib.decompress(doc_file.read(doc[1])).decode()
        return text
